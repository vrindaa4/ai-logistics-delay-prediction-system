...
const ManageUser = () => {
    ...
    const fetchUserData = async () => {
        const res = await fetch('http://localhost:1002/user/getall');
    ...
    const deleteUser = async (id) => {
        ...
        const res = await fetch('http://localhost:1002/user/delete/' + id, { method: 'DELETE' });
            ...
            <tbody>
                {
                    userList.map((user) => (
                        <tr className='p-0' >
                            ...
                            <td>{user.password}</td>
                            <td>{user.avatar}</td>
                            <td className='text-center'> <img width={50} className="img-fluid  rounded-5" src={"http://localhost:1002/" + user.avatar} alt="" /></td>
